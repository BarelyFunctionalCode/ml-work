...
class ImaBust(object):
  ...
  def restart(self):
    # self.state = np.full((self.state_1_d_size, self.state_1_d_size), self.min_value, dtype=np.int64)
    self.state = np.random.randint(self.min_value, (self.max_value - self.min_value) // 2, (self.state_1_d_size, self.state_1_d_size))

  def apply_action(self, action=None):
    i = None
    if action is not None:
      # increment selected cell and score
      i = np.unravel_index(action, self.state.shape)
      self.state[i] += 1

      # Draw next game frame
      self._render_frame()

    return self.state, {"action_coord": i, "action_value": self.state[i]}
  ...
  def _render_frame(self):
    ...
    for y, row in enumerate(self.state):
      for x, cell in enumerate(row):
        colors = {
          self.max_value - 1: (0, 255, 0),
          self.max_value: (255, 0, 0),
          "default": (0, 0, 255)
        }
        text = self.font.render(str(cell.item()), True, colors.get(cell.item(), colors["default"]))
        text_list.append(
          {
            "text": text,
            "pos": ((x * pix_square_size) + (pix_square_size / 2) - 10,
                    (y * pix_square_size) + (pix_square_size / 2) - 10)
          }
        )

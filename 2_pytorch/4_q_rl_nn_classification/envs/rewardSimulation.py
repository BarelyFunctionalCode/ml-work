...
class RewardSimulation(object):
  ...
  def _generate_reward(self, action):
      ...
      (
        (
          ...
          ( # Get ratio of current steps by max possible steps
            ...
            (
              (self.obs_size) * (self.max_value - self.min_value - 2) # Max possible steps
            )
          ) # Flip ratio
        ) ** (1. / 2) # Square Root it
      ) - 0.5 # Scaled from -0.5 - 0.5 instead of 0 - 1
    ...
    if np.array_equal(new_obs, np.full(self.obs_size, self.max_value - 1)):
      reward = 2.0
      terminated = True
      self.wins += 1
...
while not terminated:
  random_index = np.random.choice(np.arange(state_size)[sim.state < (max_value - 1)])
  if sim.state[random_index] == max_value - 1: continue
